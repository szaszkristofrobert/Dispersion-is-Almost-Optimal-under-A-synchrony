...
class UI:
    def __init__(self, graph):
        self.graph = graph
        self.window = ctk.CTk()
        # ... (az init többi része változatlan)
    ...
    def draw_graph(self):
        ...
        for node_num, node in self.graph.nodes.items():
            ...
            # Ha van itt aktív csoport, jelezzük sárga körrel a csomópont mellett
            if active_count > 0:
                self.graph_pane.create_oval(node.x + 15, node.y - 15, node.x + 25, node.y - 5, fill="yellow")
                self.graph_pane.create_text(node.x + 20, node.y - 10, text=str(active_count), font=("Arial", 7))
    ...
    # ... (A többi segédfüggvény változatlan: on_node_left_click, safe_save, stb.)
    def mainloop(self):
    ...
    def safe_save(self):
        ...
        if fn: self.graph.save_graph(fn); self.write_to_terminal(f"Saved: {fn}")

    def safe_load(self):
        fn = self.filename_entry.get().strip()
        if fn: self.graph.load_graph(fn); self.draw_graph(); self.write_to_terminal(f"Loaded: {fn}")
    ...
    def on_node_right_click(self, event):
        num = self.find_node_number_at(event.x, event.y)
        if num: self.graph.add_agent(int(num)); self.draw_graph()
    ...
    def find_node_number_at(self, x, y):
        ...
        for item in items:
            for tag in self.graph_pane.gettags(item):
                if tag.startswith("node_text_"): return tag.split("_")[2]
        return None
